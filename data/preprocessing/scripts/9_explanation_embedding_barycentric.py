...
class BarycentricEmbeddingProcessor:
    ...
    def _compute_statistics(self):
        """Compute statistics for each metric directly from explanation-level data.

        Statistics are computed from all explanation rows (3 per feature) to ensure
        standardization captures the full variance of individual explainer scores.
        """
        logger.info("Computing metric statistics from explanation-level data...")
        logger.info(f"Using {len(self.merged_df):,} explanation rows for statistics")

        # Compute statistics directly from explanation-level data
        for metric in self.metrics_order:
