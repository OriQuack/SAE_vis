...
def load_config(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    Also merges config_sources.yaml if present (auto-generated by step_00).
    ...
    # Merge auto-generated config_sources.yaml if present
    sources_path = config_path.parent / "config_sources.yaml"
    if sources_path.exists():
        config = _merge_config_sources(config, sources_path)

    return config


def _merge_config_sources(config: Dict[str, Any], sources_path: Path) -> Dict[str, Any]:
    """Merge auto-generated config_sources.yaml into pipeline config.

    Updates global settings (sae_id, data_sources, llm_explainer_mapping,
    num_features) and step_02 inputs from the SAE metadata discovered by step_00.
    Appends to existing values rather than replacing them.

    Args:
        config: Main pipeline configuration
        sources_path: Path to config_sources.yaml
    ...
    sae_config = sources.get("sae_config", {})

    # Update SAE identifiers
    if sae_config.get("sae_id"):
        global_config["sae_id"] = sae_config["sae_id"]
    if sae_config.get("sae_id_sanitized"):
        global_config["sae_id_sanitized"] = sae_config["sae_id_sanitized"]

    # Update num_features
    if sae_config.get("num_features"):
        global_config.setdefault("processing", {})["num_features"] = sae_config["num_features"]
        processing = global_config["processing"]
        processing.setdefault("feature_range", {})["end"] = sae_config["num_features"]

    # Append data sources (avoid duplicates)
    existing_sources = set(global_config.get("data_sources", []))
    for src in sources.get("data_sources", []):
        if src not in existing_sources:
            global_config.setdefault("data_sources", []).append(src)

    # Merge explainer mapping
    existing_mapping = global_config.get("llm_explainer_mapping", {})
    for key, value in sources.get("llm_explainer_mapping", {}).items():
        if key not in existing_mapping:
            existing_mapping[key] = value
    global_config["llm_explainer_mapping"] = existing_mapping

    # Update step_02 inputs from SAE metadata
    if sae_config.get("hf_repo_id") and sae_config.get("hf_folder_id"):
        steps = config.setdefault("steps", {})
        step_02 = steps.setdefault("step_02_decoder_similarity", {})
        step_02_inputs = step_02.setdefault("inputs", {})
        step_02_inputs["model"] = sae_config["hf_repo_id"]
        step_02_inputs["position"] = sae_config["hf_folder_id"]

    # Update file_pattern in step_04 with correct layer number
    if sae_config.get("layer") is not None:
        layer = sae_config["layer"]
        steps = config.setdefault("steps", {})
        step_04 = steps.setdefault("step_04_explanation_embeddings", {})
        step_04_params = step_04.setdefault("parameters", {})
        step_04_params["file_pattern"] = f"layers.{layer}_latent*.txt"

    logger.info(f"Merged config_sources.yaml: {sources.get('data_sources', [])}")
    return config
    ...
    def resolve_dict(d: Dict) -> Dict:
        ...
        for key, value in d.items():
            if isinstance(value, dict):
                ...
                for v in value:
                    if isinstance(v, str):
                        resolved_list.append(resolve_string(v))
                    elif isinstance(v, dict):
                        resolved_list.append(resolve_dict(v))
                    else:
