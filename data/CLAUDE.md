...
- `clusters` (List[Struct]) - Nested cluster data with:
  ...
  - `phrases` - List of all phrases in cluster with `start_char`/`end_char` offsets
...
# Returns phrases with start_char/end_char offsets for highlighting
phrases = extract_all_phrases(explanations, method="smart")
# -> [(phrase_text, explanation_idx, phrase_idx, start_char, end_char), ...]
...
**Pipeline Version**: 5.2 (Phrase Extraction + Consensus Char Offsets + consensus_score in SVM)
